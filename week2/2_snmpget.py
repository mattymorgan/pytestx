...
def check_save(data):
    ...
    print "Previous Uptime:" + str(prev_uptime) 
    ...
    if running_last_changed < running_last_saved:
        ...
        if startup_last_changed == 0:
            print "Startup Last Changed is 0, so rebooted since last save"
            
        if running_last_changed == 0:
            print "Running Last Changed is 0, so rebooted since last save"
    else:
        print "Warning: Configuration not saved to NVRAM"
